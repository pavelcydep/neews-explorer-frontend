export default class NewsCard {
  constructor(list, conteinerText, mainApi, createCardFunction, date, site) {
    this.listContainerConservedCards = list.querySelector('.list__container-add');
  ...
  if (event.target.className === 'list__rectangle_delete') {
    this.mainApi.removeArticle(this.data._id)
      .then((data) => {
            ...
            if (res.ok) {
              return res.json().then((json) => {
                ...
                if (json.length === 0) {
                  ...
                  this.containerTextBold.add('conteiner-text__subtitle-bold_display-none');
                }
                json.forEach((i) => {
        ...
        return data.text().then((json) => Promise.reject(json));
      }).catch((e) => console.log(e));
  }
}
}
